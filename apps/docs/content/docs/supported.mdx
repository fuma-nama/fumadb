...
<Callout title="Note for Drizzle ORM">
  FumaDB's Drizzle adapter does not support MSSQL or CockroachDB yet (use Kysely for those).
  One adapter covers both majors: pass `schema` on 0.x, or `relations` from `defineRelations()` on 1.x.
</Callout>
