...
- **Drizzle ORM** (full support, query mode required; drizzle-orm `0.44+` / `0.45+` and `1.x` / RC)
- **Prisma ORM** (full support)
- **TypeORM** (via Kysely, no MongoDB support)

<Callout title="Note for Drizzle ORM">
  FumaDB's Drizzle adapter does not support CockroachDB yet (use Kysely for it). One adapter covers
  both majors: pass `schema` on 0.x, or `relations` from `defineRelations()` on 1.x. MSSQL requires
  drizzle-orm 1.x, and uses `schema` (relations v1) since Drizzle's MSSQL dialect doesn't support
  relations v2 yet.
</Callout>
