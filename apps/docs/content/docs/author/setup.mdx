...
Create a FumaDB instance, each instance manages the different versions of a schema, with its own versioning cycle:
...
The consumer should call the `configure()` function. Then, your library can receive the FumaDB client and perform further actions.

```ts title="lib/chat.ts"
import { ChatDB, myLibrary } from "your-library";

export const client = ChatDB.configure({
  provider: "mysql",
  type: "kysely",
